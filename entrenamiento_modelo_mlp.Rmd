...
#saveRDS(lmProfile, file = "lmProfile.rds")
...
registerDoParallel(cl)
system.time(
  mlpKerasDropout_tiff <-train(x_train[rfe.predictors],
                                 x_train[[31]],
...
model_validation_mlp <- predict(mlpKerasDropout_tiff,x_train[rfe.predictors])
# Calculamos el RMSE
...
# Utilizamos el conjunto de evaluación para comprobar la eficiencia del modelo
model_validation_mlp <- predict(mlpKerasDropout_tiff,x_test[rfe.predictors])
# Calculamos el RMSE
...
image_predict_mlp <- predict(mlpKerasDropout_tiff, prueba_dataframe[rfe.predictors])
